...
import { getStatusMood, STATUS_VISIBILITIES } from "@/lib/status-constants"
...
export function CampusStatusCard({ status, viewerId, compact = false }: { status: CampusStatusData; viewerId: string; compact?: boolean }) {
  ...
  return (
    <article className={cn("relative overflow-hidden border-2 border-[#191914] bg-[#fffaf0] text-[#191914] shadow-[4px_4px_0_rgba(25,25,20,0.16)] dark:border-[#f5f0e5] dark:bg-[#191914] dark:text-[#f5f0e5]", compact ? "p-4" : "p-5 sm:p-6")}>
      <span aria-hidden className={cn("absolute inset-y-0 left-0 w-1.5", mood.color)} />
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          <Link href={`/profile/${status.user.id}`}><UserAvatar name={status.user.name} image={status.user.image} role={status.user.role} size={compact ? "sm" : "md"} /></Link>
          <div className="min-w-0">
            ...
            <p className="mt-1 font-mono text-[8px] font-bold tracking-[0.12em] text-[#918b80]">{mood.english} / {mood.label}</p>
          </div>
        </div>
        <span className={cn("shrink-0 border border-[#191914] px-2 py-1 font-mono text-[8px] font-bold text-[#191914]", mood.color)}>{mood.label}</span>
      </div>
